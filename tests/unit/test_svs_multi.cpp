...
protected:
    ...
    SVSIndexBase *CastToSVS_Multi(VecSimIndex *index) {
...
TYPED_TEST(SVSMultiTest, vector_add_multiple_test) {
    ...
    auto *svs_index = this->CastToSVS_Multi(index);

    // Adding multiple vectors under the same label
    std::vector<TEST_DATA_T[dim]> v(rep);
    for (size_t i = 0; i < rep; i++) {
        GenerateVector<TEST_DATA_T>(v[i], dim, i);
    }

    std::vector<size_t> ids(rep, 46);
    ASSERT_EQ(svs_index->addVectors(v.data(), ids.data(), rep), rep);
    ...
    std::vector<size_t> id{46};
    ASSERT_EQ(svs_index->deleteVectors(id.data(), 1), rep);
...
TYPED_TEST(SVSMultiTest, empty_index) {
    ...
    ASSERT_EQ(svs_index->deleteVectors(v.data(), 1), 0);
    ...
    // Try to remove it.
    v[0] = 1;
    ASSERT_EQ(svs_index->deleteVectors(v.data(), 1), 3);
...
//TYPED_TEST(SVSMultiTest, search_more_than_there_is) {
    ...
    //for (size_t i = 0; i < n_labels; i++) {
        ...
        //auto ids = svs_index->labelLookup->at(element_label);
...
//TYPED_TEST(SVSMultiTest, preferAdHocOptimization) {
    ...
    //for (auto &comb : combinations) {
        ...
        //svs_index->curElementCount = index_size;
        //vecsim_stl::vector<idType> vec(index->getAllocator());
        //for (size_t i = 0; i < label_count; i++) {
            //svs_index->labelLookup.emplace(i, vec);
        //}
...
//TYPED_TEST(SVSMultiTest, removeVectorWithSwaps) {
    //size_t dim = 4;
    //size_t n = 6;
    ...
    //VecSimIndex_Free(index);
//}
//TYPED_TEST(SVSMultiTest, remove_vector_after_replacing_block) {
    //size_t dim = 4;
    //size_t bs = 2;
    //size_t n = 6;
    ...
    //ASSERT_EQ(svs_index->getExternalLabel(0), 1);
    //ASSERT_EQ(svs_index->getExternalLabel(1), 2);
    //ASSERT_EQ(svs_index->getExternalLabel(2), 2);
    //// checking the blob swaps.
    //ASSERT_EQ(*(TEST_DATA_T *)(svs_index->getDataByInternalId(0)), 0);
    //ASSERT_EQ(*(TEST_DATA_T *)(svs_index->getDataByInternalId(1)), 5);
    //ASSERT_EQ(*(TEST_DATA_T *)(svs_index->getDataByInternalId(2)), 4);
...
TYPED_TEST(SVSMultiTest, sanity_reinsert_1280) {
    size_t n_labels = 5;
    size_t per_label = 3;
    size_t d = 1280;
    size_t k = 5;
...
//TYPED_TEST(SVSMultiTest, test_query_runtime_params_user_build_args) {
    //size_t n = 100;
    //size_t n_labels = 25;
    //size_t per_label = 4;
    //size_t d = 4;
...
//TYPED_TEST(SVSMultiTest, svs_delete_entry_point) {
    //size_t n = 10000;
    //size_t per_label = 5;
    //size_t dim = 4;
    //size_t M = 2;
...
//TYPED_TEST(SVSMultiTest, MultiBatchIteratorHeapLogic) {
    //size_t n = 4;
    //size_t n_labels = 3;
    //size_t dim = 4;
    //size_t n_res;
...
//TYPED_TEST(SVSMultiTest, markDelete) {
    //size_t n_labels = 100;
    //size_t per_label = 10;
    //size_t k = 11;
    //size_t dim = 4;
    ...
    //ASSERT_EQ(svs_index->markDelete(0),
    ...
    //auto verify_res = [&](size_t id, double score, size_t idx) {
        ...
        //auto ids = svs_index->labelLookup.at(id);
        //for (size_t j = 0; j < ids.size(); j++) {
            //// Verifying that each vector is labeled correctly.
            //// ID is calculated according to insertion order.
    ...
    //for (labelType label = 0; label < n_labels; label++) {
        //if (label % 2 == ep_reminder) {
            ...
            //ASSERT_EQ(svs_index->markDelete(label), expected_deleted_ids);
        //}
    //}

    //ASSERT_EQ(svs_index->getNumMarkedDeleted(), n / 2);
    //ASSERT_EQ(VecSimIndex_IndexSize(index), n);
    //ASSERT_EQ(svs_index->indexLabelCount(), n_labels / 2);
    ...
    //for (size_t level = 0; level <= svs_index->getGraphDataByInternalId(n)->toplevel;
         //level++) {
        //ElementLevelData &level_data = svs_index->getElementLevelData(n, level);
    ...
    //ASSERT_EQ(svs_index->indexLabelCount(), n_labels + 1);
...
TEST(SVSMultiTest, svs_not_supported) {
    SVSParams params = {
